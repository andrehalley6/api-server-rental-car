...
class Restapi extends CI_Controller {
	...
	public function rentals($id = NULL) {
		if($this->request_method == "POST") {
			...
			$car = $this->cars->get_car_by_id($this->input->post('car-id'))->result_array();
			$client = $this->clients->get_client_by_id($this->input->post('client-id'))->result_array();
			...
			$client_rent_car = $this->rentals->get_rental_by_client_id_and_rent_date($this->input->post('client-id'), $this->input->post('car-id'), $this->input->post('date-from'), $this->input->post('date-to'))->result_array();
			...
			$car_not_available = $this->rentals->get_rental_by_car_id_and_rent_date($this->input->post('car-id'), $this->input->post('date-from'), $this->input->post('date-to'))->result_array();
		...
		elseif($this->request_method == "PUT") {
			...
			// * Car id and client id must be exists.
			$car = $this->cars->get_car_by_id($put_data['car-id'])->result_array();
			$client = $this->clients->get_client_by_id($put_data['client-id'])->result_array();
			if(empty($car) || empty($client)) {
				echo deliver_response(400, "fail", "Car id and client id must be exists.");
				exit;
			}
			...
			// * Client is not rent another car at selected rent date.
			$client_rent_car = $this->rentals->get_rental_by_client_id_and_rent_date($put_data['client-id'], $put_data['car-id'], $put_data['date-from'], $put_data['date-to'])->result_array();
			if($client_rent_car || !empty($client_rent_car)) {
				echo deliver_response(400, "fail", "Client is rented another car at selected rent date.");
				exit;
			}
			...
			// * Car is not rented at selected rent date.
			$car_not_available = $this->rentals->get_rental_by_car_id_and_rent_date($put_data['car-id'], $put_data['date-from'], $put_data['date-to'])->result_array();
			if(!empty($car_not_available) || $car_not_available) {
				echo deliver_response(400, "fail", "Car is rented at selected rent date.");
				exit;
			}
			...
			// Check current id on database
			$car = $this->rentals->get_rental_by_id($put_data['id'])->result_array();
			if(empty($car)) {
				echo deliver_response(400, "ok", "The ID must be exists on the database");
				exit;
			}
			...
			if($this->rentals->edit_rental($put_data['id'], $put_data)) {
				echo car_rental_response("status", "Update successful.");
				exit;
			}
			else {
		...
		elseif($this->request_method == "DELETE") {
			...
			// Check current id on database
			$rental = $this->rentals->get_rental_by_id($delete_data['id'])->result_array();
			if(empty($rental)) {
				echo deliver_response(400, "ok", "The ID must be exists on the database");
				exit;
			}
			...
			if($this->rentals->delete_rental($delete_data['id'])) {
				echo car_rental_response("status", "Delete successful.");
				exit;
			}
			else {
		...
		elseif($this->request_method == "GET") {
			...
			$rentals = $this->rentals->get_all_rentals()->result_array();
